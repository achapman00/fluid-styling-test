...
Chart.plugins.register({
  beforeDraw: function(chartInstance, easing) {
    ...
    ctx.fillStyle = '#FCFCFC'; // your color here
...
const zoomToCounty = function(county) {
    ...
    var county = (county.length < 5) ? '0' + county : county;

    // console.log('Zooming map to county ID: ' + county);

    var relatedFeatures = map.querySourceFeatures('countyLayer',{
    ...
    if(relatedFeatures[0]) {
    	...
    	var bounds = bbox(relatedFeatures[0].toJSON());
	...
	if(window.app.global.vars.chart.comparison_type) {
		...
		let secondaryYAxis = {
			...
			ticks: {
				padding:20,
				callback: (value) => {
					switch(window.app.global.vars.chart.comparison_format) {
						case 'percentage':
							value = value + '%';
							break;
						default:
...
$(window).on('vf_search_selection_completed', () => {
	...
	if(window.app.global.vars.searchSelections.selection_1) {
		...
		_expDataLayer.push({'schema': 'add_event', 'version': 'v2', 'data': {
...
module.exports = async function() {
	...
	var configureMapLayers = () => {
		...
		// 	source:{
		// 		type:"vector",
		// 		url:"mapbox://kenmanz0.6ioyofdc"
		// 	},
		// 	"source-layer":"gz_2010_us_states_040_00_500k-bcw286",
		// 	layout:{
		// 		visibility:"visible"
		// 	},paint:{
		// 		"line-color":"#f9f9f9"
	...
	$('[data-search-input]').on('keyup',function(e) {
		...
		let html = [];
		results.forEach((v,i) => {
			html.push('<li><a data-search-autocomplete-select-type="' + v.obj.type + '" data-search-autocomplete-select="' + v.obj.value + '">' + w.str.toTitleCase(v.obj.name) + '</a></li>');
		});
		$autocomplete
			.html(html.join(''))
	...
	$('[data-search-input]').on('blur',function(e) {
